...
export const ProjectWizard: React.FC<ProjectWizardProps> = ({ navigate }) => {
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(() => {
      ...
      // Navigate to ideaToCode with the new project context
      navigate('ideaToCode', { 
        initialProject: {
          name: formData.name,
        ...
        return (
          ...
          >
            <div className="text-center">
              ...
              <p className="text-slate-400">اشرح فكرة مشروعك باختصار ليتمكن الذكاء الاصطناعي من فهمها</p>
            </div>
    ...
    <div className="max-w-2xl mx-auto py-12 px-4 h-full flex flex-col justify-center">
      ...
      <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden">
        ...
        <div className="mt-12 flex items-center justify-between gap-4">
          ...
          <button
            onClick={nextStep}
            disabled={isSubmitting || (step === 2 && !formData.name) || (step === 3 && !formData.description)}
            className={`flex items-center gap-2 px-8 py-3 rounded-2xl font-bold transition-all shadow-xl ${
              isSubmitting || (step === 2 && !formData.name) || (step === 3 && !formData.description)
          ...
          >
            {isSubmitting ? (
              ...
              <>
                {step === 3 ? 'ابدأ بناء المشروع' : 'التالي'}
                {step === 3 ? <RocketLaunchIcon className="w-5 h-5" /> : <ArrowLeftIcon className="w-5 h-5" />}
              </>
            )}
          </button>
        </div>
      ...
      <div className="mt-8 text-center">
        <p className="text-slate-500 text-sm">
          يمكنك دائماً تعديل هذه التفاصيل لاحقاً من لوحة تحكم المشروع
        </p>
      </div>
    </div>
  );
