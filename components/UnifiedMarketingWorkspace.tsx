  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-right" dir="rtl">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            {userEditRequest.trim().length > 0 && (
              <button
                onClick={() => setUserEditRequest('')}
                className="absolute left-3 bottom-3 text-[10px] text-slate-500 hover:text-white px-2 py-1 bg-slate-900 border border-slate-800 rounded-lg hover:bg-slate-850 transition-all"
              >
                مسح المدخلات
              </button>
            )}
        ...
        {/* Super Action Grid of 8 Buttons requested by user */}
        <div className="space-y-4">
          <span className="text-[11px] text-slate-400 font-bold block mb-1">🚀 إجراءات الأزار الثمانية الفائقة للمستخدم:</span>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
            
            {/* Button 1: طلب تقديم إقتراحات وتوصيات */}
            <button
            ...
            >
              ...
              <div>
                <h5 className="font-extrabold text-xs text-white">💡 تقديم اقتراحات وتوصيات</h5>
                <p className="text-[9px] text-slate-500 leading-normal mt-0.5">دراسة مصفوفة النمو وتحسين الحملة بـ Gemini 1.5</p>
              </div>
            </button>

            {/* Button 2: تغيير وتحميل نبرات الصوت والصوتيات الخاصة بالإعلان */}
            <button
            ...
            >
              ...
              <div>
                <h5 className="font-extrabold text-xs text-white">🎙️ تعديل نبرة الصوت والصوتيات</h5>
                <p className="text-[9px] text-slate-500 leading-normal mt-0.5">تبديل صوت الدبلجة ورفع تعليق مخصص (.mp3)</p>
              </div>
            </button>

            {/* Button 3: زر التحميل والتنزيل للحقيبة */}
            <button
              onClick={handleTriggerMockDownload}
              disabled={loadingDownload}
              className="p-3 bg-slate-950 hover:bg-slate-900 border border-slate-800 hover:border-emerald-500/50 rounded-2xl transition-all text-right group flex flex-col justify-between"
            >
              <div className="w-8 h-8 rounded-lg bg-emerald-500/10 text-emerald-400 flex items-center justify-center mb-2">
                <Download className="w-4 h-4 animate-bounce" />
              </div>
              <div>
                <h5 className="font-extrabold text-xs text-white">⬇️ تحميل وتنزيل ملفات الحملة</h5>
                <p className="text-[9px] text-slate-500 leading-normal mt-0.5">الحصول الفوري على حزمة التصميم والنصوص المجهزة</p>
              </div>
            </button>

            {/* Button 4: زر التعديل الإقناعي لتطبيق الموجه */}
            <button
              onClick={performCampaignAdjustment}
              disabled={isAnalyzing || !userEditRequest.trim()}
              className="p-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl transition-all text-right flex flex-col justify-between shadow-xl shadow-indigo-600/10 disabled:opacity-40"
            >
              <div className="w-8 h-8 rounded-lg bg-white/10 text-white flex items-center justify-center mb-2">
                <Edit3 className="w-4 h-4" />
              </div>
              <div>
                <h5 className="font-black text-xs">✏️ هضم وتطبيق التعديل الفوري</h5>
                <p className="text-[9px] text-indigo-200 leading-normal mt-0.5">إعادة صياغةCampaign بالذكاء الاصطناعي</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <h5 className="font-extrabold text-xs text-white">💾 حفظ الأصول المعاونة</h5>
                <p className="text-[9px] text-slate-500 leading-normal mt-0.5">مزامنة مخرجات الحملة كروابط داخلية بالمشروع</p>
              </div>
            </button>

            {/* Button 6: زر المشاركة الأصلي (داخل وخارج المنصة) */}
            <button
            ...
            >
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-500 leading-normal mt-0.5">خيارات نشر الحملة للشركاء والمستهلكين مباشرة</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <h5 className="font-extrabold text-xs text-white font-mono">✕ حذف وتصفير الإعدادات</h5>
                <p className="text-[9px] text-slate-500 leading-normal mt-0.5">البدء الفوري من جديد ومسح الذاكرة الحالية</p>
              </div>
            </button>

            {/* Simulated Additional Button: Launch Plan Calendar summary info */}
            <div className="p-3 bg-slate-950 border border-slate-850 rounded-2xl text-right flex flex-col justify-between">
              <div className="w-8 h-8 rounded-lg bg-slate-900 text-slate-400 flex items-center justify-center mb-2">
                📋
              </div>
              <div>
      ...
      {/* A. Suggestions and strategical recommendations Drawer (طلب تقديم إقتراحات وتوصيات) */}
      {showSuggestionsModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-905 border border-slate-800 rounded-3xl p-6 max-w-lg w-full space-y-4 shadow-2xl animate-in zoom-in-95 duration-200 text-right">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setShowSuggestionsModal(false)} className="text-slate-500 hover:text-white font-bold">&times;</button>
            </div>
            ...
            {loadingSuggestions ? (
              ...
              <div className="space-y-3">
                {suggestions.map((s, i) => (
                  <div key={i} className="bg-slate-950 p-4 border border-slate-850 rounded-2xl text-xs leading-relaxed text-slate-200">
                    {s}
                  </div>
                ))}
              </div>
            ...
            <button 
              ...
              className="w-full py-2.5 bg-slate-900 border border-slate-800 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition-all"
            >
              إغلاق النافذة
            </button>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl text-right animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-slate-850 pb-3">
              <h4 className="text-sm font-black text-white flex items-center gap-1.5">
                ...
                <span>🎙️ إعداد المعلق الصوتي والموسيقى الخلفية للمشهد:</span>
              </h4>
            ...
            {/* Presets Grid */}
            <div className="space-y-3">
              <span className="text-xs text-slate-400 font-bold block">1. اختر المعلق ونبرة دبلجة الإعلان:</span>
              <div className="grid grid-cols-2 gap-2">
                {[
                  { id: 'gulf_luxury', label: '👑 خليجية ملكية فخمة', desc: 'مناسبة للمشاريع الريادية الراقية' },
                  { id: 'egypt_active', label: '🔥 عامية مصرية نشطة', desc: 'حماسية خفيفة ملائمة لعامة الجماهير' },
                  { id: 'shami_warm', label: '❤️ شامية ودودة هادئة', desc: 'لطيفة للتطبيقات والصحة والمدونات' },
                  { id: 'fussha_professional', label: '💼 فصحى مهنية رصينة', desc: 'رسمية ومقنعة للشركات والحلول' }
                ].map(item => (
                  <button
                    key={item.id}
                    onClick={() => {
                      setSelectedVoiceTone(item.id);
                      showToast(`🎙️ تم تحديث نبرة الصوت والمذيع الافتراضي بنجاح إلى: ${item.label}`);
                    }}
                    className={`p-2.5 rounded-xl border text-right transition-all text-xs ${selectedVoiceTone === item.id ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-950 border-slate-850 text-slate-400 hover:bg-slate-900'}`}
                  >
            ...
            <div className="space-y-3">
              <span className="text-xs text-slate-400 font-bold block">2. الموسيقى التصويرية المرافقة (Soundtracks):</span>
              <select
                value={selectedBgMusic}
                onChange={(e) => {
                  setSelectedBgMusic(e.target.value);
                  showToast('🎵 تم دمج وتحديث اللحن التسويقي المرافق بنجاح!');
                }}
                className="w-full p-2.5 bg-slate-950 border border-slate-850 rounded-xl text-xs text-white focus:outline-none"
              >
                <option value="tech_rising">إيقاع تقني صاعد ومنسجم (Tech Beat)</option>
                <option value="cinematic_inspire">سينمائي ملهم وعميق (Cinematic Glow)</option>
            ...
            <div className="space-y-2">
              <span className="text-xs text-slate-400 font-semibold block">3. أو سجّل/ارفع ملف تعليق صوتي مخصص (.mp3) للإعلان:</span>
              <div 
            ...
            >
              موافق وتعيين المكونات الصوتية
            </button>
          </div>
        </div>
      )}

      {/* C. Download campaign progress dialog */}
      {loadingDownload && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-sm w-full space-y-4 text-center animate-in zoom-in-95 duration-250">
            <div className="w-10 h-10 rounded-full bg-emerald-550/15 text-emerald-400 flex items-center justify-center mx-auto animate-bounce">
              <Download className="w-5 h-5" />
            </div>
            
            <div className="space-y-1">
              <h5 className="font-black text-sm text-white">جاري جمع وتعبية الأصول التسويقية...</h5>
              <p className="text-[10px] text-slate-400 leading-relaxed">يرجى الانتظار لحين صياغة الأصول وضغط نصوص الإشهار بمؤشرات Veo 3 و Imagen 3.</p>
            </div>

            {/* Simulated bar progress bar */}
            <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-slate-850">
              <div className="bg-emerald-500 h-full transition-all duration-100 ease-linear" style={{ width: `${downloadProgress}%` }} />
            </div>
            <span className="text-xs font-mono font-bold text-emerald-400">{downloadProgress}%</span>
          </div>
        </div>
      )}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-sm w-full space-y-4 text-right animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-slate-850 pb-3">
              <h4 className="text-sm font-black text-white flex items-center gap-1.5">
                ...
                <span>🔗 خيارات مشاركة الرابط الإعلاني الأصلي:</span>
              </h4>
              <button onClick={() => setShowShareModal(false)} className="text-slate-500 hover:text-white font-bold">&times;</button>
            </div>

            {/* Inner platform copy */}
            <div className="bg-slate-950 p-3.5 border border-slate-850 rounded-2xl text-xs space-y-2">
              <span className="text-[9px] text-indigo-400 font-bold block">الرابط المباشر للمعاينة الحركية:</span>
              <p className="text-slate-450 text-[10px] truncate select-all text-left font-mono">
                {window.location.origin}/marketing/campaign/{selectedProject?.id || 'demo'}
              </p>
              <button
                onClick={() => {
                  navigator.clipboard.writeText(`${window.location.origin}/marketing/campaign/${selectedProject?.id || 'demo'}`);
                  showToast('📋 تم نسخ الرابط المشفر بنجاح لمشاركته داخل وخارج منصتك!');
                }}
                className="w-full py-2 bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-400 border border-indigo-500/20 rounded-xl text-xs font-bold transition-all"
              >
                نسخ الرابط للمشاركة
              </button>
            </div>
            ...
            <button 
              ...
              className="w-full py-2.5 bg-slate-905 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition-all border border-slate-800"
            >
              إلغلاق
            </button>
          </div>
